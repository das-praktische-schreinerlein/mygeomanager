...
export class GeoDocConverterModule {
    ...
    constructor(backendConfig: BackendConfigType, dataService: GeoDocDataService) {
        this.dataService = dataService;
        this.backendConfig = backendConfig
        if (isArray(backendConfig.gdocImportConverterAdditionalKeywords)) {
            this.keywordSrcLst = this.keywordSrcLst.concat(backendConfig.gdocImportConverterAdditionalKeywords);
        }
    }
            ...
            for (let i = 0; i < features.length; i++) {
                ...
                for (let url of urls) {
                    ...
                    if (!typeMap['INFO'][baseUrl]) {
                        ...
                        const infoRecord =
                            <GeoDocRecord>responseMapper.mapResponseDocument(mapper, infoValues, {});
                ...
                const poiObj = <GeoDocRecord>responseMapper.mapResponseDocument(mapper, values, {});
                records.push(poiObj);
            }
